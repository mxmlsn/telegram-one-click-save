...
function _renderCardInner(item) {
  ...
  const domain = (rawDomain === 'stash.mxml.sn' || rawDomain === 'viewer note' || rawDomain === 'viewer upload') ? '' : rawDomain;
...
const typeTrigger = document.getElementById('ctx-type-trigger');
  ...
  function enterSelectMode() {
    ...
    STATE.selectedIds = new Set();
    document.getElementById('masonry').classList.add('select-mode');
  ...
  function exitSelectMode() {
    STATE.selectMode = false;
    STATE.selectedIds = new Set();
    document.getElementById('masonry').classList.remove('select-mode');
  ...
  selectDeleteBtn.addEventListener('click', async () => {
    ...
    const confirmed = confirm(`Delete ${ids.length} item${ids.length > 1 ? 's' : ''}?`);
    if (!confirmed) return;
    exitSelectMode();
    for (const id of ids) {
      await deleteItem(id);
